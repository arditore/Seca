...
- **A contact who leaves**: turning Seca Link off sends a farewell through the open sessions and asks the relays to drop the published keys (NIP-09), so the other side ends the encrypted conversation at once — a notification, a notice in the thread, no more lock, and messages back to SMS. Taking Seca off the phone announces nothing, so the keys carry a one-day expiration (NIP-40) and are published again every four hours: they fade by themselves. The listening service asks every relay once an hour, in a single question covering every contact, and opening a conversation asks again on the spot. A relay that answers nothing concludes nothing, and a contact whose keys come back is connected again.
