            ...
            itemsIndexed(link.relays, key = { _, url -> url }) { index, url ->
                SecaGroupItem(index = index, count = link.relays.size) {
                    SecaSettingRow(
                        ...
                        subtitle = statusText(link.statuses[url], link.enabled),
                        trailing = {
...
private fun statusText(status: RelayStatus?, enabled: Boolean): String = when {
    !enabled -> "Seca Link est désactivé"
